...
function registrarUsuario() {
  ...
  promise.always(function (data) {
    ...
    if (data.res == "register true") {
    ...
    } else if (data.res == "register invalid") {
      //Si no es exitoso
      ocultarSpinner();
      ...
      mensaje.innerHTML = `<div class="mt-2 border-danger text-center alert alert-danger p-2" id="mensaje">
                ...
                <b>Error:</b> no se ha verificado el reCaptcha.
            </div>`;
    } else {
      //Por si los datos son corruptos u otra cosa en vez de hacer que el cliente espere
      ocultarSpinner();
      window.alert("Error");
    }
  });
...
function iniciarSesion() {
  ...
  promise.always(function (data) {
    ...
    } else if (data.res == "login invalid") {
      //Si no es exitoso
      ocultarSpinner();
      mensaje.innerHTML = `<div class="mt-2 border-danger text-center alert alert-danger p-2" id="mensaje">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
    ...
    } else if (data.res == "login unconfirmed") {
      //Ha faltado un parametro
      ocultarSpinner();
      mensaje.innerHTML = `<div class="mt-2 border-danger text-center alert alert-danger p-2" id="mensaje">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
    ...
    } else if (data.res == "login timeout") {
      ocultarSpinner();
      mensaje.innerHTML = `<div class="mt-2 border-danger text-center alert alert-danger p-2" id="mensaje">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
                          <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5m.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2"/>
                      </svg>
                      <b>Error:</b> se han agotado los intentos de inicio de sesión. Espere unos segundos.
                  </div>`;
    } else {
      //Por si los datos son corruptos u otra cosa en vez de hacer que el cliente espere
      ocultarSpinner();
...
function resetPass() {
  ...
  promise.always(function (data) {
    mostrarSpinner();
    if (data.res == "email true") {
      //Si el login es exitoso
      ocultarSpinner();
      window.location.replace("/login?res=true");
    } else if (data.res == "email invalid") {
      //Si no es exitoso
      ocultarSpinner();
      mensaje.innerHTML = `<div class="mt-2 border-danger text-center alert alert-danger p-2" id="mensaje">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
...
function validarCamposLogin() {
  ...
  if (
    usuario.value.trim() !== "" &&
    !/\s/.test(usuario.value.trim()) &&
    usuario.value.trim().length > 5 &&
...
function validarCamposRegister() {
  ...
  if (
    ...
    passPattern.test(contrasena.value.trim()) &&
    usuario.value.trim().length > 5
  ) {
    // Si todos los campos tienen valor y el email tiene el formato correcto, habilitar el botón
    button.disabled = false;
...
function validarCampoReset() {
  var emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (
    emailReset.value.trim() !== "" &&
    emailPattern.test(emailReset.value.trim())
  ) {
    button.disabled = false;
  } else {
    button.disabled = true;
